...
public class Updatestime extends HttpServlet {
	private static final long serialVersionUID = 1L;
   	...
   	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String s = "update election set start = ?";
			PreparedStatement st = con.prepareStatement(s);
			st.setTimestamp(1, Timestamp.valueOf(newtime));
			int result = st.executeUpdate();
			if(result>0) {
				HttpSession session = request.getSession();
				session.setAttribute("stime", newtime);
				response.sendRedirect("welcome_admin.jsp");
				return;
			}
			out.println("<meta http-equiv = 'refresh' content='3; URL= update_stime.jsp'>");
